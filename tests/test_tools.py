...
from src.tools import history_query, knowledge_search, visual_search, vlm_analyze
...
# ── 사이클 3: T4 vlm_analyze ─────────────────────────────


class RecordingVLM:
    """전송된 messages를 캡처하고 정해진 관찰 JSON을 돌려주는 페이크 gpt-4o."""
    ...
    def __call__(self, messages):
        ...
        return json.dumps({"observation": "섬유질 결이 관찰됨", "confidence": "high"},
                          ensure_ascii=False)

    def sent_images(self):
        """messages에서 image_url 파트를 PIL 이미지로 복원."""
        images = []
    ...
    def prompt_text(self):
        ...
        for msg in self.messages:
            ...
            if isinstance(content, str):
                ...
                chunks += [p["text"] for p in content if p["type"] == "text"]
        return "\n".join(chunks)


def test_vlm_sends_crop_not_full_image():
    """13. cropped_bbox 지정 → 그 영역만 전송. 원본(8256px) 통짜 금지 (R5)."""
    vlm = RecordingVLM()
    result = vlm_analyze(QUERY_IMAGE, "결함이 보이는가?", cropped_bbox=(100, 200, 640, 480),
                         vlm_client=vlm)
    assert "error" not in result
    sent = vlm.sent_images()
    assert len(sent) == 1
    assert sent[0].size == (640, 480), "cropped_bbox 영역이 아닌 이미지가 전송됨"


def test_vlm_falls_back_to_thumbnail():
    """14. cropped_bbox 없으면 긴 변 기준 축소본 폴백 — 역시 원본 통짜 금지 (R5)."""
    vlm = RecordingVLM()
    vlm_analyze(QUERY_IMAGE, "결함이 보이는가?", vlm_client=vlm)
    sent = vlm.sent_images()
    assert len(sent) == 1
    assert max(sent[0].size) < 8256, "원본 통짜가 전송됨"
    assert max(sent[0].size) <= 1024


def test_vlm_few_shot_changes_prompt():
    """15. few-shot 사례 주입 유무 → 프롬프트에 라벨·설명문 포함/미포함 (핵심 가설의 연결)."""
    case = {"filename": "2024_sungsan_5_A_LeadingEdge_002.jpg",
            "defect_type": "Paint Damage", "severity": 2,
            "description": "표면 도장이 벗겨져 하부가 드러남"}

    with_shot = RecordingVLM()
    vlm_analyze(QUERY_IMAGE, "결함 종류는?", few_shot=[case], vlm_client=with_shot)
    assert "Paint Damage" in with_shot.prompt_text()
    assert "표면 도장이 벗겨져" in with_shot.prompt_text()

    without = RecordingVLM()
    vlm_analyze(QUERY_IMAGE, "결함 종류는?", vlm_client=without)
    assert "Paint Damage" not in without.prompt_text()


def test_vlm_compare_mode_sends_two_images():
    """16. 비교 모드(UC-5) — 두 번째 사진 지정 시 이미지 2장이 함께 전송된다."""
    other = STUB_DATA_DIR / "2024_sungsan_5_A_LeadingEdge_002.jpg"
    vlm = RecordingVLM()
    result = vlm_analyze(QUERY_IMAGE, "작년 사진 대비 손상이 커졌는가?",
                         compare_image=other, vlm_client=vlm)
    assert len(vlm.sent_images()) == 2
    assert result["observation"] and result["confidence"] in ("high", "medium", "low")
    assert result["params"]["compare_image"] == other.name  # R7
