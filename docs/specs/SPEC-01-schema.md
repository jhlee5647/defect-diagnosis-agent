...
```jsonc
{
  ...
  "vision_qa":   { "object_description": "앞전 중앙부에 페인트 박리가...",
                   "defect_classification_q": "이 결함의 유형은?", "...4지선다 보기·정답...": "",
...
| R7 | 필수 블록(info, image, collection, categories, vision_qa)이나 설명문 없으면 `ValueError` | 깨진 파일이 인덱스에 들어가면 검색 결과에 빈 캡션이 섞임. categories 없으면 결함 번호를 이름으로 풀 수 없음 (SPEC-02 R4의 전제) |
